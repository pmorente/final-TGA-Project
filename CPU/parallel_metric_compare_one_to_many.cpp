...
#include <omp.h>
#include <stdexcept>
#include "cosine/cosine_parallel.hpp"
#include "pearson/pearson_parallel.hpp"
#include "euclidean/euclidean_parallel.hpp"
...
// ------------------ One-to-Many Comparison Function (Parallel at Metric Level) -------------------
void compare_one_to_many(const string& file_a, const string& file_b, MetricType metric, bool save_results = false) {
    ...
    int embedding_dim = records_a.empty() ? 0 : static_cast<int>(records_a[0].embedding.size());
    
    string metric_name = (metric == COSINE) ? "cosine" : (metric == EUCLIDEAN) ? "euclidean" : "pearson";
    cout << "\n=== Computing One-to-Many Comparisons (Parallel at Metric Level) ===" << endl;
    ...
    cout << "Using " << num_threads << " threads for metric computation (parallelizing dimension loops)" << endl;
    
    // Results storage
    std::vector<float> results(records_a.size() * records_b.size());
    
    // Compute similarities: parallelism is inside metric computation functions
    ...
    cout << "Number of threads: " << num_threads << " (used for dimension-level parallelism)" << endl;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 4) {
        ...
        cerr << "\nThis is STRATEGY 3: Parallel at METRIC COMPUTATION level." << endl;
        cerr << "Parallelism is applied to the inner loops over vector dimensions (like CUDA reduction)." << endl;
        cerr << "\nExample:" << endl;
        cerr << "  " << argv[0] 
...
// ------------------ Metric Computation (using parallel versions) -------------------
float compute_metric(MetricType metric, const vector<float>& A, const vector<float>& B) {
    if (A.size() != B.size()) {
    ...
    switch (metric) {
        case COSINE:
            return cosine_cpu_parallel(A, B);
        case EUCLIDEAN:
            return euclidean_distance_cpu_parallel(A.data(), B.data(), static_cast<int>(A.size()));
        case PEARSON:
            return pearson_corr_cpu_parallel(A.data(), B.data(), static_cast<int>(A.size()));
        default:
