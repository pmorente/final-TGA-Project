...
// ------------------ One-to-Many Comparison Function (Parallel at Vector Level) -------------------
void compare_one_to_many(const string& file_a, const string& file_b, MetricType metric, bool save_results = false) {
    ...
    cout << "\n=== Computing One-to-Many Comparisons (Parallel at Vector Level) ===" << endl;
    ...
    // Progress tracking
    atomic<size_t> progress_count(0);
    mutex progress_mutex;
    
    // Compute similarities: parallelize outer loop over vectors
    ...
    for (size_t i = 0; i < records_a.size(); ++i) {
        for (size_t j = 0; j < records_b.size(); ++j) {
            ...
            size_t current_count = progress_count.fetch_add(1) + 1;
            
            // Print progress (with mutex to avoid interleaved output)
            if (current_count % 1000 == 0 || current_count == total_comparisons) {
                lock_guard<mutex> lock(progress_mutex);
                cout << "  Progress: " << current_count << "/" << total_comparisons 
                     << " (" << fixed << setprecision(1) << (100.0 * current_count / total_comparisons) << "%)" << endl;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 4) {
        ...
        cerr << "\nThis is STRATEGY 2: Parallel at VECTOR LEVEL using OpenMP." << endl;
        cerr << "\nExample:" << endl;
        cerr << "  " << argv[0] 
