...
// Algorithm: Pearson Correlation (Standard Two-Pass)
// Optimization: Hybrid Reduction (Loop + Warp Volatile) to reduce register pressure
// ============================================================================
...
__global__ void pearson_strategy5_template(
    ...
    int dim)
{
    ...
    // [0 ... dim-1] -> s_query (Vector A - Persistente)
    // [dim ...]     -> Buffers reusables para reducciones
    extern __shared__ float s[];
    float* s_query = s;
    float* s_buff1 = s_query + dim;
    ...
    // =========================================================
    // FASE 0: Cargar A en Shared Memory
    // =========================================================
    ...
    // =========================================================
    // FASE 1: Pre-calcular Estadísticas de A (SOLO UNA VEZ)
    // =========================================================
    
    // 1.1 Calcular Media de A
    float thread_sumA = 0.0f;
    for (int d = tid; d < dim; d += BLOCK_SIZE) {
        thread_sumA += s_query[d];
    }
    ...
    for (unsigned int stride = BLOCK_SIZE / 2; stride > 32; stride >>= 1) {
        if (tid < stride) s_buff1[tid] += s_buff1[tid + stride];
        __syncthreads();
    }
    if (tid < 32) {
    ...
    // 1.2 Calcular Denominador de A (Varianza)
    float thread_varA = 0.0f;
    for (int d = tid; d < dim; d += BLOCK_SIZE) {
        float diff = s_query[d] - meanA;
        thread_varA += diff * diff;
    }
    s_buff1[tid] = thread_varA;
    __syncthreads();

    // Reducción Varianza A
    for (unsigned int stride = BLOCK_SIZE / 2; stride > 32; stride >>= 1) {
        if (tid < stride) s_buff1[tid] += s_buff1[tid + stride];
        __syncthreads();
    }
    ...
    __shared__ float denomA;
    if (tid == 0) denomA = sqrtf(s_buff1[0]);
    __syncthreads(); 


    // =========================================================
    // FASE 2: Bucle Principal (Solo operamos con B)
    // =========================================================
    ...
    for (int db_idx = 0; db_idx < num_database; db_idx++) {
        ...
        // ---------------------------------------------------
        // PASO 2.1: Calcular Media de B
        // ---------------------------------------------------
        float thread_sumB = 0.0f;
        for (int d = tid; d < dim; d += BLOCK_SIZE) {
            thread_sumB += db_vec[d];
        }
        ...
        // ---------------------------------------------------
        // PASO 2.2: Covarianza y Denominador B
        // ---------------------------------------------------
        float thread_num = 0.0f;
        float thread_denB = 0.0f;
        
        for (int d = tid; d < dim; d += BLOCK_SIZE) {
            float valA = s_query[d] - meanA; // meanA pre-calculada
            float valB = db_vec[d] - meanB;
            
            thread_num  += valA * valB;
            thread_denB += valB * valB;
        }

        s_buff1[tid] = thread_num;
        s_buff2[tid] = thread_denB;
        __syncthreads(); // BARRERA 2

        // Reducción Final
        for (unsigned int stride = BLOCK_SIZE / 2; stride > 32; stride >>= 1) {
            if (tid < stride) {
                s_buff1[tid] += s_buff1[tid + stride];
                s_buff2[tid] += s_buff2[tid + stride];
            }
            __syncthreads();
        }
        if (tid < 32) {
            volatile float* sm1 = s_buff1;
            volatile float* sm2 = s_buff2;
            if (BLOCK_SIZE > 32) {
                sm1[tid] += sm1[tid + 32];
                sm2[tid] += sm2[tid + 32];
            }
        ...
        if (tid == 0) {
            ...
            float total_denB = sqrtf(s_buff2[0]);
            float total_denA = denomA;
            
            float divisor = total_denA * total_denB;
            if (divisor > 1e-12f) {
                results[q_idx * num_database + db_idx] = total_num / divisor;
            } else {
...
void launch_pearson_template(int bs, const float* A, const float* B, float* C, int nA, int nB, int dim) {
    dim3 grid(nA); dim3 block(bs); size_t sh = (dim + 2 * bs) * sizeof(float);
    if (bs==32) pearson_strategy5_template<32><<<grid, block, sh>>>(A,B,C,nA,nB,dim);
    else if (bs==64) pearson_strategy5_template<64><<<grid, block, sh>>>(A,B,C,nA,nB,dim);
    else if (bs==128) pearson_strategy5_template<128><<<grid, block, sh>>>(A,B,C,nA,nB,dim);
    else if (bs==256) pearson_strategy5_template<256><<<grid, block, sh>>>(A,B,C,nA,nB,dim);
    else if (bs==512) pearson_strategy5_template<512><<<grid, block, sh>>>(A,B,C,nA,nB,dim);
    else if (bs==1024) pearson_strategy5_template<1024><<<grid, block, sh>>>(A,B,C,nA,nB,dim);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "\n=== Strategy 5: 2D Tiled (TEMPLATE HYBRID) ===" << std::endl;
        std::cout << "Algorithm: Pearson" << std::endl;
