...
// KERNEL PEARSON
// ============================================================================
__global__ void pearson_strategy1(
    ...
    int dim)
{
    ...
    // First pass: compute means (sequential)
    float meanA = 0.0f;
    float meanB = 0.0f;
    for (int k = 0; k < dim; k++) {
        meanA += query[k];
    ...
    // Second pass: compute correlation (sequential)
    float num = 0.0f;
    float denA = 0.0f;
    float denB = 0.0f;
    for (int k = 0; k < dim; k++) {
        float da = query[k] - meanA;
        float db = db_vec[k] - meanB;
        num += da * db;
        denA += da * da;
        denB += db * db;
    }
    
    float denom = sqrtf(denA * denB) + 1e-12f;
    results[q_idx * num_database + db_idx] = num / denom;
}

void launch_pearson_strategy1(
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "\n=== Strategy 1: Grid over pairs, Pearson Benchmark ===" << std::endl;
        ...
        launch_pearson_strategy1(d_queries, d_database, d_results, num_queries, num_database, dim);
