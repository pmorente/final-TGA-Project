#!/bin/bash
# ============================================================================
# GPU Strategy 6 Benchmark Script (Bash version for WSL)
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Note: Strategy 6 uses C++ templates. Supported block sizes: 32, 64, 128, 256, 512, 1024"
    exit 1
fi
...
if [ $((BLOCK_SIZE & (BLOCK_SIZE - 1))) -ne 0 ]; then
    ...
    echo "Strategy 6 requires power-of-2 block sizes for template specialization."
fi
...
# Configuration
GPU_DIR="$(cd ../GPU/strategy_6 && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
INPUT_DIR="$PROJECT_ROOT/inputs"
DIMENSIONS=(384 768 1024)
METRICS=("cosine" "euclidean" "pearson" "pearson_opt")
RESULTS_FILE="$SCRIPT_DIR/strategy6_results_block${BLOCK_SIZE}.md"

# Check if GPU executables exist
echo ""
echo "=== Checking GPU Executables ==="
for metric in "${METRICS[@]}"; do
    exe="$GPU_DIR/estrategia6_${metric}"
    if [ ! -f "$exe" ]; then
        echo "Error: Missing executable: $exe"
        echo "Please compile first with:"
        echo "  cd $GPU_DIR && nvcc -O3 -arch=sm_89 estrategia6_${metric}.cu -o estrategia6_${metric}"
        exit 1
    fi
done
...
echo "Strategy 6 uses C++ templates for compile-time optimization."
...
run_benchmark() {
    ...
    local exe="$GPU_DIR/estrategia6_${metric}"
    ...
    cd "$GPU_DIR"
    output=$(./estrategia6_${metric} "$fileA" "$fileB" "$BLOCK_SIZE" 2>&1)
...
content="# GPU Strategy 6 Benchmark Results - RTX 3050

Generated on: $timestamp

**Block Size:** $BLOCK_SIZE

**Template Option:** Strategy 6 uses C++ templates for compile-time optimization. The kernel is automatically specialized for block size $BLOCK_SIZE at compile-time, providing optimal performance through:
...
**Note:** Strategy 6 is the inverse mapping of Strategy 5 - grid over BIG group (database), loop SMALL group (queries).
