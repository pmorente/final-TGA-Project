...
// Algorithm: Pearson Correlation (One-Pass Formula)
// Features: Benchmark Mode (Warmup + 10 runs)
// Mapping: 1 thread = 1 vector comparison
...
// ============================================================================
__global__ void pearson_strategy1_opt(
    ...
    int dim)
{
    // blockIdx.x → index in A (queries)
    // blockIdx.y → index in B (database)
    int q_idx = blockIdx.x;
    int db_idx = blockIdx.y;
    
    if (q_idx >= num_queries || db_idx >= num_database) return;
    
    const float* query = queries + q_idx * dim;
    const float* db_vec = database + db_idx * dim;
    
    // One-pass: compute all statistics in a single loop
    float sumA = 0.0f;
    float sumSqA = 0.0f;
    float sumB = 0.0f;
    float sumSqB = 0.0f;
...
void launch_pearson_strategy1_opt(
    ...
    int dim)
{
    dim3 grid(num_queries, num_database);  // 2D grid
    dim3 block(1);                         // 1 thread per block
    
    pearson_strategy1_opt<<<grid, block>>>(
        d_queries, d_database, d_results,
        num_queries, num_database, dim
    );
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "\n=== Strategy 1 (One-Pass): Grid over pairs, Pearson Benchmark ===" << std::endl;
        ...
        launch_pearson_strategy1_opt(d_queries, d_database, d_results, num_queries, num_database, dim);
