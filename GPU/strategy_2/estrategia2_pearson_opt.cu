// ============================================================================
// STRATEGY 2 — Grid over SMALL group, loop BIG group, sequential computation
// Algorithm: Pearson Correlation (One-Pass Formula, Sequential per thread)
// Features: Benchmark Mode (Warmup + 10 runs)
// Formula: r = (N*sumAB - sumA*sumB) / sqrt((N*sumSqA - sumA²) * (N*sumSqB - sumB²))
// ============================================================================
...
// ============================================================================
// KERNEL PEARSON (ONE-PASS)
// ============================================================================
__global__ void pearson_strategy2_opt(
    ...
    int BLOCK_SIZE)
{
    // blockIdx.x = i → vector A[i] (query)
    int q_idx = blockIdx.x;
    if (q_idx >= num_queries) return;
    
    const float* query = queries + q_idx * dim;
    
    // Pre-calculate query statistics (one-pass)
    float sumA = 0.0f;
    float sumSqA = 0.0f;
    for (int k = 0; k < dim; k++) {
        float a = query[k];
    ...
    // Each thread handles multiple B[j] vectors
    int tid = threadIdx.x;
    int num_threads = blockDim.x;
    
    for (int db_idx = tid; db_idx < num_database; db_idx += num_threads) {
        const float* db_vec = database + db_idx * dim;
        
        // One-pass: compute all statistics for this comparison
        float sumB = 0.0f;
        float sumSqB = 0.0f;
        float sumAB = 0.0f;
        ...
        // One-pass Pearson formula
        float N = (float)dim;
        float num = N * sumAB - sumA * sumB;
        float varA = N * sumSqA - sumA * sumA;
        float varB = N * sumSqB - sumB * sumB;
        float denom = sqrtf(varA * varB) + 1e-12f;
        
        results[q_idx * num_database + db_idx] = num / denom;
    }
}

void launch_pearson_strategy2_opt(
    ...
    int BLOCK_SIZE)
{
    ...
    pearson_strategy2_opt<<<grid, block>>>(
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "\n=== Strategy 2 (One-Pass): Grid over SMALL group, Pearson Benchmark ===" << std::endl;
        ...
        launch_pearson_strategy2_opt(d_queries, d_database, d_results, num_queries, num_database, dim, block_size);
