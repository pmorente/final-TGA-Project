...
__global__ void cosine_strategy7(const float* __restrict__ A, const float* __restrict__ B, float* __restrict__ C, int num_A, int num_B, int dim) {
    int TILE = blockDim.x;
...
void launch_cosine_strategy7(int tile_size, const float* d_A, const float* d_B, float* d_C, int num_A, int num_B, int dim) {
    dim3 grid((num_B + tile_size - 1) / tile_size, (num_A + tile_size - 1) / tile_size);
    dim3 block(tile_size, tile_size);
    size_t shmem = 2 * tile_size * tile_size * sizeof(float);
    cosine_strategy7<<<grid, block, shmem>>>(d_A, d_B, d_C, num_A, num_B, dim);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        launch_cosine_strategy7(tile_size, d_A, d_B, d_C, num_queries, num_database, dim);
