...
// Algorithm: Cosine Similarity (Sequential per thread, no reduction)
...
// KERNEL COSINE
// ============================================================================
__global__ void cosine_strategy2(
    ...
    int BLOCK_SIZE)
{
    ...
    for (int db_idx = tid; db_idx < num_database; db_idx += num_threads) {
        ...
        float denom = sqrtf(normA) * sqrtf(normB) + 1e-12f;
        results[q_idx * num_database + db_idx] = dot / denom;
    }
}

void launch_cosine_strategy2(
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "\n=== Strategy 2: Grid over SMALL group, Cosine Benchmark ===" << std::endl;
        ...
        launch_cosine_strategy2(d_queries, d_database, d_results, num_queries, num_database, dim, block_size);
