#!/bin/bash
# ============================================================================
# GPU Strategy 3 Block Size Comparison Script
...
RESULTS_FILE="$SCRIPT_DIR/strategy3_blocksize_comparison.md"

# Check if GPU executables exist
echo ""
echo "=== Checking GPU Executables ==="
for metric in "${METRICS[@]}"; do
    exe="$GPU_DIR/estrategia3_${metric}"
    if [ ! -f "$exe" ]; then
        echo "Error: Missing executable: $exe"
        echo "Please compile first with:"
        echo "  cd $GPU_DIR && nvcc -O3 -arch=sm_89 estrategia3_${metric}.cu -o estrategia3_${metric}"
        exit 1
    fi
done
...
run_benchmark() {
    ...
    local exe="$GPU_DIR/estrategia3_${metric}"
    ...
    cd "$GPU_DIR"
    output=$(./estrategia3_${metric} "$fileA" "$fileB" "$block_size" 2>&1)
...
content="# GPU Strategy 3 Block Size Comparison - RTX 3050

Generated on: $timestamp

This document compares performance across different block sizes (32, 64, 128, 256, 512, 1024) for Strategy 3.
