...
// ============================================================================
__global__ void pearson_strategy3_opt(
    ...
    int BLOCK_SIZE)
{
    ...
    // Pre-calculate database vector statistics (one-pass)
    float sumB = 0.0f;
    float sumSqB = 0.0f;
    for (int k = 0; k < dim; k++) {
        float b = db_vec[k];
    ...
    // Each thread processes multiple A[i] vectors
    int tid = threadIdx.x;
...
void launch_pearson_strategy3_opt(
    ...
    int BLOCK_SIZE)
{
    ...
    pearson_strategy3_opt<<<grid, block>>>(
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "\n=== Strategy 3 (One-Pass): Grid over BIG group, Pearson Benchmark ===" << std::endl;
        ...
        launch_pearson_strategy3_opt(d_queries, d_database, d_results, num_queries, num_database, dim, block_size);
