#!/bin/bash
# ============================================================================
# GPU Strategy 2 Block Size Comparison Script
...
RESULTS_FILE="$SCRIPT_DIR/strategy2_blocksize_comparison.md"

# Check if GPU executables exist
echo ""
echo "=== Checking GPU Executables ==="
for metric in "${METRICS[@]}"; do
    exe="$GPU_DIR/estrategia2_${metric}"
    if [ ! -f "$exe" ]; then
        echo "Error: Missing executable: $exe"
        echo "Please compile first with:"
        echo "  cd $GPU_DIR && nvcc -O3 -arch=sm_89 estrategia2_${metric}.cu -o estrategia2_${metric}"
        exit 1
    fi
done
...
parse_output() {
    ...
    while IFS= read -r line; do
        if [[ $line =~ Avg[[:space:]]+Computation[[:space:]]+time:[[:space:]]+([0-9.]+)[[:space:]]+seconds ]]; then
...
run_benchmark() {
    ...
    local exe="$GPU_DIR/estrategia2_${metric}"
    ...
    cd "$GPU_DIR"
    output=$(./estrategia2_${metric} "$fileA" "$fileB" "$block_size" 2>&1)
...
for dim in "${DIMENSIONS[@]}"; do
    ...
    for metric in "${METRICS[@]}"; do
        for block_size in "${BLOCK_SIZES[@]}"; do
            run_benchmark "$metric" "$dim" "$block_size"
        done
    done
done
...
content="# GPU Strategy 2 Block Size Comparison - RTX 3050

Generated on: $timestamp

This document compares performance across different block sizes (32, 64, 128, 256, 512, 1024) for Strategy 2.
    ...
    for metric_name in "Cosine" "Euclidean" "Pearson" "Pearson Opt"; do
        ...
        for i in "${!BLOCK_SIZES[@]}"; do
            ...
            if [ -n "${results_avg_throughput[$key]}" ]; then
                ...
                row+=" $(printf "%10.2f" "$throughput_val") |"
...
for dim in "${DIMENSIONS[@]}"; do
    ...
    for metric_name in "Cosine" "Euclidean" "Pearson" "Pearson Opt"; do
        ...
        for block_size in "${BLOCK_SIZES[@]}"; do
            ...
            if [ -n "${results_avg_throughput[$key]}" ]; then
                throughput_val="${results_avg_throughput[$key]}"
                row+=" $(printf "%4.2f" "$throughput_val") |"
            else
                row+=" $(printf "%10s" "ERROR") |"
            fi
        done
