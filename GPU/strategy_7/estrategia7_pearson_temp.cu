...
template <int TILE>
__global__ void pearson_strategy7_template(const float* __restrict__ A, const float* __restrict__ B, float* __restrict__ C, int num_A, int num_B, int dim) {
    ...
    if (row < num_A && col < num_B) {
        float num = dim * prod - sumA * sumB;
        float denA = dim * sqA - sumA * sumA;
        float denB = dim * sqB - sumB * sumB;
        float denom = sqrtf(denA) * sqrtf(denB);
        C[row * num_B + col] = (denom > 1e-12f) ? num / denom : 0.0f;
    }
}

void launch_pearson_template(int tile_size, const float* d_A, const float* d_B, float* d_C, int num_A, int num_B, int dim) {
    if (tile_size == 4) {
        dim3 grid((num_B+3)/4, (num_A+3)/4); dim3 block(4,4);
        pearson_strategy7_template<4><<<grid, block>>>(d_A, d_B, d_C, num_A, num_B, dim);
    } else if (tile_size == 8) {
        dim3 grid((num_B+7)/8, (num_A+7)/8); dim3 block(8,8);
        pearson_strategy7_template<8><<<grid, block>>>(d_A, d_B, d_C, num_A, num_B, dim);
    } else if (tile_size == 16) {
        dim3 grid((num_B+15)/16, (num_A+15)/16); dim3 block(16,16);
        pearson_strategy7_template<16><<<grid, block>>>(d_A, d_B, d_C, num_A, num_B, dim);
    } else if (tile_size == 30) {
        dim3 grid((num_B+29)/30, (num_A+29)/30); dim3 block(30,30);
        pearson_strategy7_template<30><<<grid, block>>>(d_A, d_B, d_C, num_A, num_B, dim);
    } else if (tile_size == 31) {
        dim3 grid((num_B+30)/31, (num_A+30)/31); dim3 block(31,31);
        pearson_strategy7_template<31><<<grid, block>>>(d_A, d_B, d_C, num_A, num_B, dim);
    } else if (tile_size == 32) {
        dim3 grid((num_B+31)/32, (num_A+31)/32); dim3 block(32,32);
        pearson_strategy7_template<32><<<grid, block>>>(d_A, d_B, d_C, num_A, num_B, dim);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        launch_pearson_template(tile_size, d_A, d_B, d_C, num_queries, num_database, dim);
